...
public final class FileAgentTools {
  private static final String PREF="axtor_files";
  ...
  public static String execute(Context c,String action,JSONObject args)throws Exception{
    switch(action){
      case "LIST_FILES": return list(c,args.optString("path",""));
      case "SEARCH_FILES": return search(c,args.optString("query",""));
      case "READ_FILE": return read(c,args.optString("path",""));
      case "WRITE_FILE": return write(c,args.optString("path",""),args.optString("content",""),false);
      case "APPEND_FILE": return write(c,args.optString("path",""),args.optString("content",""),true);
      case "RENAME_FILE": return rename(c,args.optString("path",""),args.optString("new_name",""));
      case "CREATE_DIRECTORY": return mkdir(c,args.optString("path",""));
      case "DELETE_FILE": return delete(c,args.optString("path",""));
      default:return null;
    }
  }

  private static DocumentFile root(Context c){String u=c.getSharedPreferences(PREF,0).getString(TREE,"");if(!u.isEmpty()){DocumentFile d=DocumentFile.fromTreeUri(c,Uri.parse(u));if(d!=null&&d.canRead())return d;}return DocumentFile.fromFile(new File(c.getExternalFilesDir(null),"AxtorWorkspace"));}
  private static DocumentFile ensureRoot(Context c){DocumentFile r=root(c);if(!r.exists())r.getUri();return r;}
  private static DocumentFile resolve(Context c,String path,boolean createDirs)throws IOException{
    String p=path==null?"":path.trim().replace('\\','/');while(p.startsWith("/"))p=p.substring(1);if(p.startsWith("..")||p.contains("/../"))throw new IOException("Unsafe path");
    DocumentFile cur=ensureRoot(c);if(p.isEmpty())return cur;
    for(String part:p.split("/")){if(part.isEmpty()||part.equals("."))continue;DocumentFile next=null;for(DocumentFile f:cur.listFiles())if(f.getName()!=null&&f.getName().equals(part)){next=f;break;}if(next==null){if(!createDirs)throw new FileNotFoundException(p);next=cur.createDirectory(part);}cur=next;}
    return cur;
  }
  ...
  private static String mkdir(Context c,String path)throws Exception{DocumentFile d=resolve(c,path,true);return "Directory ready: "+path;}
