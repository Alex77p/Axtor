...
public class MainActivity extends Activity {
  ...
  void customAutomation(){clear();content.addView(tv("Custom Automation",22));content.addView(tv("Your calling phrase is already configured in Voice. Do NOT enter it here. Write only the words you will say after the calling phrase. Example: if your calling phrase is “computer”, enter “turn the volume down”, not “computer turn the volume down”.",14));content.addView(tv("Axtor automatically combines your saved calling phrase + this command when listening.",13));EditText trigger=new EditText(this);trigger.setHint("Only the command, e.g. turn volume up");content.addView(trigger);Spinner action=new Spinner(this);String[] options={"Open Settings","Open Wi-Fi settings","Open Bluetooth settings","Go Home","Go Back","Open Recent Apps","Open Notifications","Lock Screen","Volume Up","Volume Down","Mute","Unmute","Set Alarm"};action.setAdapter(new ArrayAdapter<String>(this,android.R.layout.simple_spinner_dropdown_item,options));content.addView(action);Button save=btn("＋ Save Custom Automation");save.setOnClickListener(v->{String t=trigger.getText().toString().trim();String phrase=VoiceCommandManager.getCallPhrase(this);if(t.toLowerCase(Locale.ROOT).startsWith(phrase+" ")){t=t.substring(phrase.length()).trim();}if(t.isEmpty()){Toast.makeText(this,"Enter only the command after your calling phrase.",Toast.LENGTH_SHORT).show();return;}try{JSONArray a=AppCore.flows(this);JSONObject o=new JSONObject();o.put("trigger",t);o.put("condition",true);o.put("action",action.getSelectedItem().toString());a.put(o);AppCore.setFlows(this,a);Toast.makeText(this,"Saved: “"+t+"” → "+action.getSelectedItem().toString(),Toast.LENGTH_SHORT).show();trigger.setText("");}catch(Exception e){Toast.makeText(this,"Could not save: "+e.getMessage(),Toast.LENGTH_LONG).show();}});content.addView(save);content.addView(tv("Saved custom automations",18));JSONArray saved=AppCore.flows(this);for(int i=0;i<saved.length();i++){try{JSONObject o=saved.getJSONObject(i);TextView row=tv((i+1)+". "+o.optString("trigger")+" → "+o.optString("action"),14);content.addView(row);}catch(Exception ignored){}}Button back=btn("← Back");back.setOnClickListener(v->flows());content.addView(back);}
