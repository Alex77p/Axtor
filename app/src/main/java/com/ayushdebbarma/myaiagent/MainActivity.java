...
public class MainActivity extends Activity {
  ...
  void customAutomation(){clear();content.addView(tv("Custom Automation",22));content.addView(tv("Your calling phrase is already configured in Voice. Do NOT enter it here. Write only the words you will say after the calling phrase. Example: if your calling phrase is “computer”, enter “turn the volume down”, not “computer turn the volume down”.",14));content.addView(tv("Axtor automatically combines your saved calling phrase + this command when listening.",13));EditText trigger=new EditText(this);trigger.setSingleLine(true);trigger.setHint("Command only, e.g. open WhatsApp");content.addView(trigger);
    EditText action=new EditText(this);action.setSingleLine(true);action.setHint("Task to perform, e.g. open WhatsApp / volume down / open Wi-Fi settings");content.addView(action);
    content.addView(tv("Advanced: you choose the exact Android task. Examples: “open WhatsApp”, “volume down”, “settings display”, “url https://example.com”, or “intent android.settings.WIFI_SETTINGS”. You can also use any public Android Intent action supported by the device. Axtor does not execute arbitrary shell/code; Android permissions still apply.",13));
    Button save=btn("＋ Save Custom Automation");save.setOnClickListener(v->{String t=trigger.getText().toString().trim();String atext=action.getText().toString().trim();String phrase=VoiceCommandManager.getCallPhrase(this);String nl=t.toLowerCase(Locale.ROOT);if(nl.equals(phrase))t="";else if(nl.startsWith(phrase+" "))t=t.substring(phrase.length()).trim();if(t.isEmpty()||atext.isEmpty()){Toast.makeText(this,"Enter both a voice command and the task.",Toast.LENGTH_SHORT).show();return;}try{JSONArray a=AppCore.flows(this);JSONObject o=new JSONObject();o.put("trigger",t);o.put("condition",true);o.put("action",atext);a.put(o);AppCore.setFlows(this,a);Toast.makeText(this,"Saved: “"+t+"” → "+atext,Toast.LENGTH_SHORT).show();trigger.setText("");action.setText("");}catch(Exception e){Toast.makeText(this,"Could not save: "+e.getMessage(),Toast.LENGTH_LONG).show();}});content.addView(save);content.addView(tv("Saved custom automations",18));JSONArray saved=AppCore.flows(this);for(int i=0;i<saved.length();i++){try{JSONObject o=saved.getJSONObject(i);TextView row=tv((i+1)+". "+o.optString("trigger")+" → "+o.optString("action"),14);content.addView(row);}catch(Exception ignored){}}Button back=btn("← Back");back.setOnClickListener(v->flows());content.addView(back);}
  void flows(){clear();content.addView(tv("Automation",22));content.addView(tv("Voice/manual actions now execute supported Android actions. App navigation and global actions require enabling Axtor Accessibility Service.",14));Button custom=btn("＋ Create Custom Automation");custom.setOnClickListener(v->customAutomation());content.addView(custom);Button access=btn(AxtorAccessibilityService.isEnabled()?"✓ Accessibility service enabled":"🔐 Enable Axtor Accessibility Service");access.setOnClickListener(v->{try{startActivity(new Intent(Settings.ACTION_ACCESSIBILITY_SETTINGS));}catch(Exception e){startActivity(new Intent(Settings.ACTION_SETTINGS));}});content.addView(access);String[] acts={"Open Settings","Open Wi-Fi settings","Open Bluetooth settings","Go Home","Go Back","Open Recent Apps","Open Notifications","Lock Screen","Volume Up","Volume Down","Mute","Unmute","Set Alarm"};for(String s:acts){Button b=btn("▶ "+s);b.setOnClickListener(v->{String result=DeviceAutomation.execute(this,s);Toast.makeText(this,result==null?"Action not supported":result,Toast.LENGTH_LONG).show();});content.addView(b);}content.addView(tv("Saved flows: "+AppCore.flows(this).length(),14));Button imp=btn("📥 Import automation JSON");imp.setOnClickListener(v->{Intent i=new Intent(Intent.ACTION_OPEN_DOCUMENT);i.setType("application/json");i.addCategory(Intent.CATEGORY_OPENABLE);startActivityForResult(i,IMPORT_FLOW);});content.addView(imp);Button exp=btn("📤 Export automation JSON");exp.setOnClickListener(v->exportFile(EXPORT_FLOW,"myaiagent-automations.json"));content.addView(exp);Button back=btn("← Back");back.setOnClickListener(v->home());content.addView(back);}
