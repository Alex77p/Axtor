...
public final class AxtorReliability {
    ...
    public static void recordFailure(Context c,String component,String error){if(c==null)return;try{JSONObject o=new JSONObject().put("component",component==null?"unknown":component).put("error",error==null?"unknown":error.substring(0,Math.min(2000,error.length()))).put("time",System.currentTimeMillis());c.getSharedPreferences(PREF,0).edit().putString(FAILURE,o.toString()).apply();}catch(Exception ignored){}}
    ...
    public static void emergencyStop(Context c){if(c==null)return;AxtorTaskManager.cancel(c);try{VoiceAssistantService.stop(c);}catch(Throwable ignored){}}
}
