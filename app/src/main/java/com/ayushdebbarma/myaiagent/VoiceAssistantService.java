...
public class VoiceAssistantService extends Service implements RecognitionListener, TextToSpeech.OnInitListener {
  ...
  boolean restartScheduled=false,onlineFallback=false,usingOnDevice=false;
  ...
  @Override public void onCreate(){
    ...
    tts.setOnUtteranceProgressListener(new UtteranceProgressListener(){public void onStart(String id){} public void onDone(String id){if(running&&continuousListening())armHandsFree();} public void onError(String id){if(running&&continuousListening())armHandsFree();}});
    armHandsFree();
  }
  ...
  private void armHandsFree(){
    ...
    if(snapMode()){
      ...
      if(!SnapTriggerEngine.isEnrolled(this)){
        rememberVoiceError("SNAP_NOT_ENROLLED");
        return;
      }
      snapDetector.start();
  ...
  void startRecognition(){
    ...
    if(checkSelfPermission("android.permission.RECORD_AUDIO")!=PackageManager.PERMISSION_GRANTED){rememberVoiceError("MIC_PERMISSION_MISSING");say("Voice is unavailable. Main cause: microphone permission is missing.");stopSelf();return;}
    if(!SpeechRecognizer.isRecognitionAvailable(this)){rememberVoiceError("SPEECH_RECOGNIZER_UNAVAILABLE");say("Voice is unavailable. Main cause: no speech recognition service is installed.");stopSelf();return;}
  ...
  void command(String q){
    String cmd=extractCommand(q);if(cmd==null)return;
