...
public final class SnapTriggerEngine {
    ...
    private static final int RATE=16000, FRAME=2048, HOP=1024;
    private static final long NORMAL_COOLDOWN_MS=650, EMERGENCY_COOLDOWN_MS=250;
    ...
    public static String liveStatus(Context c){SharedPreferences p=c.getSharedPreferences(PREF,0);if(!isEnrolled(c))return "Not enrolled • enroll 3 snaps";if(p.getBoolean("emergency_armed",false))return "3 snaps detected • emergency stop armed (snap once more)";return "Ready • snaps detected: "+p.getInt("pattern_count",0)+"/4";}
    ...
    private void loop(){int min=AudioRecord.getMinBufferSize(RATE,AudioFormat.CHANNEL_IN_MONO,AudioFormat.ENCODING_PCM_16BIT);if(min<=0){listener.onDiagnostic("SNAP_AUDIO_UNAVAILABLE");running=false;return;}AudioRecord r=null;try{r=new AudioRecord(MediaRecorder.AudioSource.MIC,RATE,AudioFormat.CHANNEL_IN_MONO,AudioFormat.ENCODING_PCM_16BIT,Math.max(min,FRAME*3));if(r.getState()!=AudioRecord.STATE_INITIALIZED){listener.onDiagnostic("SNAP_AUDIO_INIT_FAILED");running=false;return;}r.startRecording();short[] rolling=new short[FRAME];short[] chunk=new short[HOP];int filled=0,calibrationFrames=0;while(running){int n=r.read(chunk,0,chunk.length);if(n!=HOP)continue;System.arraycopy(rolling,HOP,rolling,0,FRAME-HOP);System.arraycopy(chunk,0,rolling,FRAME-HOP,HOP);filled+=HOP;if(filled<FRAME)continue;Features f=features(rolling,FRAME);if(calibrationFrames<24){updateLiveNoise(f);calibrationFrames++;continue;}updateLiveNoiseQuietly(f);if(isAdaptiveSnapLike(f,liveNoiseRms,liveNoisePeak)&&matchesTemplate(f)){long now=System.currentTimeMillis();long cooldown=emergencyOnly?EMERGENCY_COOLDOWN_MS:NORMAL_COOLDOWN_MS;if(now-lastTrigger>cooldown){lastTrigger=now;registerPattern(now);}}}}catch(Throwable t){listener.onDiagnostic("SNAP_DETECTOR_ERROR:"+t.getClass().getSimpleName());}finally{if(r!=null){try{r.stop();}catch(Exception ignored){}try{r.release();}catch(Exception ignored){}}}}
    private void updateLiveNoise(Features f){liveNoiseRms=Math.max(.0005,f.rms*1.10);liveNoisePeak=Math.max(.006,f.peak*1.10);}
    private void updateLiveNoiseQuietly(Features f){double gateRms=liveNoiseRms*1.45,gatePeak=liveNoisePeak*1.55;if(f.rms<gateRms&&f.peak<gatePeak){liveNoiseRms=.97*liveNoiseRms+.03*Math.max(.0005,f.rms);liveNoisePeak=.97*liveNoisePeak+.03*Math.max(.006,f.peak);}}
    private void registerPattern(long now){
        if(emergencyOnly){if(recordEmergencySnap(now)){EmergencyStopController.request(context);persistEmergency(false);emergencyOnly=false;patternCount=0;persistPattern();listener.onDiagnostic("SNAP_EMERGENCY_STOP_EXECUTED");}return;}
        ...
        patternCount=Math.min(4,patternCount+1);persistPattern();final int count=patternCount;
        listener.onDiagnostic("SNAP_COUNT:"+count+"/4");
        new android.os.Handler(android.os.Looper.getMainLooper()).postDelayed(()->{
            if(!running||patternCount!=count)return;
            patternCount=0;persistPattern();
            if(count==1)listener.onSnap();
            else if(count==2)executeConfiguredPattern("double");
            else if(count==4)executeConfiguredPattern("quad");
        },900);
        if(count==3){
            emergencyOnly=true;persistEmergency(true);listener.onDiagnostic("SNAP_PATTERN_TRIPLE_EMERGENCY_ARMED");
        }
    }
    ...
    private boolean recordEmergencySnap(long now){synchronized(recentSnaps){while(!recentSnaps.isEmpty()&&now-recentSnaps.peekFirst()>PATTERN_WINDOW_MS)recentSnaps.removeFirst();recentSnaps.addLast(now);return recentSnaps.size()>=EMERGENCY_SNAP_COUNT;}}
    ...
    /** Adaptive enrollment: calibrate local noise, ignore continuous background, and retain the strongest three transient candidates. */
    public static boolean enroll(Context c){
        setEnrollmentProgress(c,0);if(c.checkSelfPermission(Manifest.permission.RECORD_AUDIO)!=0){setEnrollmentDiagnostic(c,"SNAP_ENROLL_MIC_PERMISSION_MISSING");return false;}
        ...
        AudioRecord r=null;try{r=new AudioRecord(MediaRecorder.AudioSource.MIC,RATE,AudioFormat.CHANNEL_IN_MONO,AudioFormat.ENCODING_PCM_16BIT,Math.max(min,FRAME*3));if(r.getState()!=AudioRecord.STATE_INITIALIZED){setEnrollmentDiagnostic(c,"SNAP_ENROLL_AUDIO_INIT_FAILED");return false;}r.startRecording();short[] rolling=new short[FRAME],chunk=new short[HOP];final int calibrationFrames=32;int filled=0,frames=0,candidates=0;double noiseRms=.001,noisePeak=.01;List<Candidate> best=new ArrayList<>();long end=System.currentTimeMillis()+20000;setEnrollmentDiagnostic(c,"SNAP_ENROLL_CALIBRATING_NOISE");while(System.currentTimeMillis()<end&&best.size()<3){int n=r.read(chunk,0,chunk.length);if(n!=HOP)continue;System.arraycopy(rolling,HOP,rolling,0,FRAME-HOP);System.arraycopy(chunk,0,rolling,FRAME-HOP,HOP);filled+=HOP;if(filled<FRAME)continue;frames++;Features f=features(rolling,FRAME);if(frames<=calibrationFrames){noiseRms=.90*noiseRms+.10*Math.max(.0005,f.rms);noisePeak=.90*noisePeak+.10*Math.max(.006,f.peak);if(frames==calibrationFrames)setEnrollmentDiagnostic(c,"SNAP_ENROLL_LISTENING_NOISE_RMS:"+round(noiseRms)+"_PEAK:"+round(noisePeak));continue;}double quietRms=noiseRms*1.35,quietPeak=noisePeak*1.55;if(f.rms<quietRms&&f.peak<quietPeak){noiseRms=.98*noiseRms+.02*Math.max(.0005,f.rms);noisePeak=.98*noisePeak+.02*Math.max(.006,f.peak);}if(isAdaptiveSnapLike(f,noiseRms,noisePeak)){candidates++;addCandidate(best,new Candidate(f,snapScore(f,noiseRms,noisePeak)));setEnrollmentProgress(c,best.size());setEnrollmentDiagnostic(c,"SNAP_ENROLL_CANDIDATE:"+candidates+"_BEST:"+best.size()+"/3_NOISE_RMS:"+round(noiseRms)+"_NOISE_PEAK:"+round(noisePeak));try{Thread.sleep(220);}catch(InterruptedException ignored){Thread.currentThread().interrupt();break;}}}
        ...
        double[] avg=new double[6];for(Candidate q:best)for(int i=0;i<avg.length;i++)avg[i]+=q.features.vector()[i];for(int i=0;i<avg.length;i++)avg[i]/=best.size();StringBuilder s=new StringBuilder();for(int i=0;i<avg.length;i++){if(i>0)s.append(',');s.append(avg[i]);}
        c.getSharedPreferences(PREF,0).edit().putString(TEMPLATE,s.toString()).putFloat("threshold",0.48f).putInt("pattern_count",0).putBoolean("emergency_armed",false).putFloat("noise_rms",(float)noiseRms).putFloat("noise_peak",(float)noisePeak).putInt("snap_candidates",candidates).putInt("snap_samples_captured",3).putString("last_enrollment_diagnostic","SNAP_ENROLL_SUCCESS:3/3_CANDIDATES:"+candidates).apply();setEnrollmentProgress(c,3);return true;
        }catch(Throwable t){setEnrollmentDiagnostic(c,"SNAP_ENROLL_ERROR:"+t.getClass().getSimpleName());return false;}finally{if(r!=null){try{r.stop();}catch(Exception ignored){}try{r.release();}catch(Exception ignored){}}}}
    private static boolean isAdaptiveSnapLike(Features f,double noiseRms,double noisePeak){boolean extended=ExtendedRangeState.enabled;double peakFloor=extended?.055:.09;double crestFloor=extended?1.75:2.1;double hfFloor=extended?.025:.045;double zcrFloor=extended?.008:.012;double rmsFloor=Math.max(.0025,noiseRms*1.55);double peakAdaptive=Math.max(peakFloor,noisePeak*1.65);return f.peak>peakAdaptive&&f.rms>rmsFloor&&f.crest>crestFloor&&f.hf>hfFloor&&f.zcr>zcrFloor&&f.durationMs>=100&&f.durationMs<=170;}
    ...
    private boolean matchesTemplate(Features f){SharedPreferences p=context.getSharedPreferences(PREF,0);String raw=p.getString(TEMPLATE,"");if(raw.isEmpty())return false;try{String[] a=raw.split(",");double[] t=new double[a.length];for(int i=0;i<a.length;i++)t[i]=Double.parseDouble(a[i]);double similarity=similarity(f.vector(),t);context.getSharedPreferences(PREF,0).edit().putFloat("last_similarity",(float)similarity).apply();return similarity>=p.getFloat("threshold",0.48f);}catch(Exception e){return false;}}
    ...
    private static Features features(short[] x,int n){double sum=0,peak=0;int z=0;double diffEnergy=0;for(int i=0;i<n;i++){double v=Math.abs(x[i])/32768.0;sum+=v*v;if(v>peak)peak=v;if(i>0){if((x[i]>=0)!=(x[i-1]>=0))z++;double d=(x[i]-x[i-1])/32768.0;diffEnergy+=d*d;}}double rms=Math.sqrt(sum/Math.max(1,n));double hf=diffEnergy/Math.max(4*sum,.0000001);double crest=peak/Math.max(rms,.0001);return new Features(peak,crest,Math.min(1,hf),(z/(double)n),n*1000.0/RATE,rms);}
