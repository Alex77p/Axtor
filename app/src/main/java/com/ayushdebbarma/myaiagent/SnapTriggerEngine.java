...
public final class SnapTriggerEngine {
    ...
    public static boolean enroll(Context c){
        ...
        try{
            ...
            r.startRecording(); List<double[]> samples=new ArrayList<>(); short[] b=new short[FRAME]; long end=System.currentTimeMillis()+9000;
            while(System.currentTimeMillis()<end && samples.size()<3){
                int n=r.read(b,0,b.length); if(n!=FRAME)continue;
                Features f=features(b,n);
                if(isSnapLike(f)){samples.add(f.vector());try{Thread.sleep(650);}catch(InterruptedException ignored){}}
            }
            ...
            c.getSharedPreferences(PREF,0).edit().putString(TEMPLATE,s.toString()).putFloat("threshold",0.86f).apply();
            return true;
    ...
    private boolean matchesTemplate(Features f){
        SharedPreferences p=context.getSharedPreferences(PREF,0);String raw=p.getString(TEMPLATE,"");
    ...
    private static double similarity(double[] a,double[] b){
        if(a.length!=b.length)return 0;double dot=0,aa=0,bb=0;for(int i=0;i<a.length;i++){dot+=a[i]*b[i];aa+=a[i]*a[i];bb+=b[i]*b[i];}if(aa==0||bb==0)return 0;return dot/(Math.sqrt(aa)*Math.sqrt(bb));
    }

    private static Features features(short[] x,int n){
        double sum=0,peak=0;int z=0;for(int i=0;i<n;i++){double v=Math.abs(x[i])/32768.0;sum+=v*v;if(v>peak)peak=v;if(i>0 && ((x[i]>=0)!=(x[i-1]>=0)))z++;}
        double rms=Math.sqrt(sum/n);double hf=0,total=0;for(int k=1;k<n/2;k++){double re=0,im=0;double w=2*Math.PI*k/n;for(int i=0;i<n;i+=2){double a=x[i]/32768.0;re+=a*Math.cos(w*i);im-=a*Math.sin(w*i);}double mag=re*re+im*im;total+=mag;if(k>n/8)hf+=mag;}
        double crest=peak/Math.max(rms,0.0001);return new Features(peak,crest,hf/Math.max(total,0.0001),(z/(double)n),n*1000.0/RATE,rms);
    }
