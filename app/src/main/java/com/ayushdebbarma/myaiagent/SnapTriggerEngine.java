...
public final class SnapTriggerEngine {
    ...
    private void loop(){int min=AudioRecord.getMinBufferSize(RATE,AudioFormat.CHANNEL_IN_MONO,AudioFormat.ENCODING_PCM_16BIT);if(min<=0){listener.onDiagnostic("SNAP_AUDIO_UNAVAILABLE");running=false;return;}AudioRecord r=null;try{r=new AudioRecord(MediaRecorder.AudioSource.MIC,RATE,AudioFormat.CHANNEL_IN_MONO,AudioFormat.ENCODING_PCM_16BIT,Math.max(min,FRAME*2));if(r.getState()!=AudioRecord.STATE_INITIALIZED){listener.onDiagnostic("SNAP_AUDIO_INIT_FAILED");running=false;return;}r.startRecording();short[] buf=new short[FRAME];int calibrationFrames=0;while(running){if(!VoiceServiceState.isRunning()){running=false;break;}int n=r.read(buf,0,buf.length);if(n!=FRAME)continue;Features f=features(buf,n);if(calibrationFrames<24){updateLiveNoise(f);calibrationFrames++;continue;}updateLiveNoiseQuietly(f);if(isAdaptiveSnapLike(f,liveNoiseRms,liveNoisePeak)&&matchesTemplate(f)){long now=System.currentTimeMillis();long cooldown=emergencyOnly?EMERGENCY_COOLDOWN_MS:NORMAL_COOLDOWN_MS;if(now-lastTrigger>cooldown){lastTrigger=now;registerPattern(now);}}}}catch(Throwable t){listener.onDiagnostic("SNAP_DETECTOR_ERROR:"+t.getClass().getSimpleName());}finally{if(r!=null){try{r.stop();}catch(Exception ignored){}try{r.release();}catch(Exception ignored){}}}}
    private void updateLiveNoise(Features f){liveNoiseRms=Math.max(.0008,f.rms*1.15);liveNoisePeak=Math.max(.008,f.peak*1.15);}
    private void updateLiveNoiseQuietly(Features f){double gateRms=liveNoiseRms*1.55,gatePeak=liveNoisePeak*1.7;if(f.rms<gateRms&&f.peak<gatePeak){liveNoiseRms=.97*liveNoiseRms+.03*Math.max(.0008,f.rms);liveNoisePeak=.97*liveNoisePeak+.03*Math.max(.008,f.peak);}}
    ...
    public static boolean enroll(Context c){
        ...
        AudioRecord r=null;try{r=new AudioRecord(MediaRecorder.AudioSource.MIC,RATE,AudioFormat.CHANNEL_IN_MONO,AudioFormat.ENCODING_PCM_16BIT,Math.max(min,FRAME*2));if(r.getState()!=AudioRecord.STATE_INITIALIZED){setEnrollmentDiagnostic(c,"SNAP_ENROLL_AUDIO_INIT_FAILED");return false;}r.startRecording();short[] b=new short[FRAME];final int calibrationFrames=32;int frames=0,candidates=0;double noiseRms=.001,noisePeak=.01;List<Candidate> best=new ArrayList<>();long end=System.currentTimeMillis()+20000;setEnrollmentDiagnostic(c,"SNAP_ENROLL_CALIBRATING_NOISE");while(System.currentTimeMillis()<end&&best.size()<3){int n=r.read(b,0,b.length);if(n!=FRAME)continue;frames++;Features f=features(b,n);if(frames<=calibrationFrames){noiseRms=.90*noiseRms+.10*Math.max(.0008,f.rms);noisePeak=.90*noisePeak+.10*Math.max(.008,f.peak);if(frames==calibrationFrames)setEnrollmentDiagnostic(c,"SNAP_ENROLL_LISTENING_NOISE_RMS:"+round(noiseRms)+"_PEAK:"+round(noisePeak));continue;}double quietRms=noiseRms*1.45,quietPeak=noisePeak*1.65;if(f.rms<quietRms&&f.peak<quietPeak){noiseRms=.98*noiseRms+.02*Math.max(.0008,f.rms);noisePeak=.98*noisePeak+.02*Math.max(.008,f.peak);}if(isAdaptiveSnapLike(f,noiseRms,noisePeak)){candidates++;addCandidate(best,new Candidate(f,snapScore(f,noiseRms,noisePeak)));setEnrollmentProgress(c,best.size());setEnrollmentDiagnostic(c,"SNAP_ENROLL_CANDIDATE:"+candidates+"_BEST:"+best.size()+"/3_NOISE_RMS:"+round(noiseRms)+"_NOISE_PEAK:"+round(noisePeak));try{Thread.sleep(350);}catch(InterruptedException ignored){Thread.currentThread().interrupt();break;}}}
    ...
    private static boolean isAdaptiveSnapLike(Features f,double noiseRms,double noisePeak){boolean extended=ExtendedRangeState.enabled;double peakFloor=extended?.075:.12;double crestFloor=extended?2.15:2.6;double hfFloor=extended?.045:.07;double zcrFloor=extended?.012:.018;double rmsFloor=Math.max(.0045,noiseRms*2.0);double peakAdaptive=Math.max(peakFloor,noisePeak*2.25);return f.peak>peakAdaptive&&f.rms>rmsFloor&&f.crest>crestFloor&&f.hf>hfFloor&&f.zcr>zcrFloor&&f.durationMs>=90&&f.durationMs<=180;}
    private static double snapScore(Features f,double noiseRms,double noisePeak){double peakRatio=f.peak/Math.max(noisePeak,.005),rmsRatio=f.rms/Math.max(noiseRms,.001);return Math.min(8,peakRatio)+Math.min(6,rmsRatio)+f.crest+f.hf*4+f.zcr*10;}
