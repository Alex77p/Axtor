...
/** Central Phase-6..11 router: chat, memory, bounded tasks, tools, verification and recovery. */
public final class AxtorAgent {
        ...
        try{
            ...
            HybridAiRouter.generate(context,model,prompt,system,192,new HybridAiRouter.Callback(){
                public void onSuccess(String text,double tps){
                    if(AxtorReliability.shouldAbort(context)){callback.onReply("Task cancelled.");return;}
                    String raw=text==null?"":text.trim();
                    try{
                        JSONObject actionRequest=parseAction(raw);
                        if(actionRequest!=null){
                            String canonical=canonicalModelAction(actionRequest.optString("action",""));
                            if(canonical==null){String blocked="Axtor generated an action outside the safe device-tool policy, so it was not executed.";remember(context,"assistant",blocked);callback.onReply(blocked);return;}
                            AxtorTaskManager.update(context,"executing",1,canonical);
                            String command=commandForCanonical(canonical);
                            String result=DeviceAutomation.execute(context,command);
                            if(result==null){String blocked="Axtor selected a device action that is not currently supported.";AxtorTaskManager.update(context,"failed",1,blocked);remember(context,"tool",blocked);remember(context,"assistant",blocked);callback.onReply(blocked);return;}
                            String verified=AgentExecutionVerifier.verify(context,command,result);
                            AxtorTaskManager.update(context,"completed",1,verified);
                    ...
                    if(taskIntent)AxtorTaskManager.update(context,"completed",1,"Response generated");
                    remember(context,"assistant",answer);callback.onReply(answer);
                }
                public void onError(String message){AxtorReliability.recordFailure(context,"hybrid-ai-router",message);AxtorTaskManager.update(context,"failed",0,message);AutonomousRepairCoordinator.onFailure(context,"hybrid-ai-router",message);callback.onError("AI error: "+message);}
            });
        }catch(OutOfMemoryError oom){AxtorReliability.recordFailure(context,"llama-runtime","Out of memory");AxtorTaskManager.update(context,"failed",0,"Out of memory");try{LlamaRuntime.releaseCachedModel();}catch(Throwable ignored){}AutonomousRepairCoordinator.onFailure(context,"llama-runtime","Out of memory");callback.onError("The model needs more memory. Try a smaller quantized GGUF model.");}
        catch(Throwable t){String message=t.getMessage()==null?t.getClass().getSimpleName():t.getMessage();AxtorReliability.recordFailure(context,"agent-router",message);AxtorTaskManager.update(context,"failed",0,message);AutonomousRepairCoordinator.onFailure(context,"agent-router",message);callback.onError(message);}
    }
    ...
    private static boolean looksLikeMultiStepTask(String value){String l=value.toLowerCase(Locale.ROOT);return l.matches(".*\\b(then|after that|and then|step by step|multiple steps|first .* then|plan and)\\b.*");}
    private static String buildPrompt(Context context,String current){StringBuilder b=new StringBuilder();b.append("Conversation history:\n");JSONArray h=history(context);for(int i=0;i<h.length();i++){JSONObject o=h.optJSONObject(i);if(o==null)continue;b.append(o.optString("role","user")).append(": ").append(o.optString("text","")).append('\n');}String fact=AxtorMemory.get(context,"user_fact");if(!fact.isEmpty())b.append("Explicit local memory:\nuser_fact: ").append(fact).append('\n');JSONObject task=AxtorTaskManager.active(context);if(task.length()>0)b.append("Active task state: ").append(task.toString()).append('\n');b.append("user: ").append(current).append('\n').append("assistant:");return b.toString();}
