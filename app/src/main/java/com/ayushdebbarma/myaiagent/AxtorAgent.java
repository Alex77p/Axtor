...
public final class AxtorAgent {
    ...
    public static void handle(Context context,String command,Callback callback){
        ...
        ComponentRegistry.register(context,"agent-router","2","running");
        try{
    ...
    private static void remember(Context context,String role,String text){try{android.content.SharedPreferences p=context.getSharedPreferences(PREF,0);JSONArray old=history(context),out=new JSONArray();int start=Math.max(0,old.length()-(MAX_HISTORY-1));for(int i=start;i<old.length();i++)out.put(old.opt(i));JSONObject item=new JSONObject();item.put("role",role);item.put("text",text==null?"":text);item.put("time",System.currentTimeMillis());out.put(item);p.edit().putString(HISTORY,out.toString()).apply();}catch(Exception ignored){}}
