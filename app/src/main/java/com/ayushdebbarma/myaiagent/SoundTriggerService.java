...
public class SoundTriggerService extends Service {
    ...
    private void executeConfiguredAction(String type){
        if(!running)return;
        SharedPreferences p=getSharedPreferences("axtor_sound",0);
        boolean adaptive=SnapDetectionProtocol.ADAPTIVE.equals(SnapDetectionProtocol.get(this));
        if(!adaptive&&!p.getBoolean("snap_allow_generic_actions",false)){
            p.edit().putString("last_trigger",type).putString("last_result","detected_only_non_personal_protocol").apply();
            return;
        }
        lastActionAt=System.currentTimeMillis();
        String fallback=type.equals("single")?"wake screen":type.equals("double")?"volume down":type.equals("triple")?"open settings":"stop all";
        String action=p.getString(type+"_action",fallback).trim();
        if(action.isEmpty())return;
        if("quad".equals(type))action="stop all";
        String policy=AxtorCommandSecurityPolicy.authorizeSnap(this,action);
        if(!"OK".equals(policy)){p.edit().putString("last_result","blocked:"+policy).apply();return;}
        if(p.getBoolean("wake_on_screen_off",true)&&!"stop all".equals(action)){
